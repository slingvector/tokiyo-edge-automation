...
export class InstagramEngager {
    ...
    private async findLikeButton(maxScrolls = 4): Promise<{ x: number, y: number, alreadyLiked: boolean } | null> {
        for (let i = 0; i <= maxScrolls; i++) {
            ...
            // Primary: resource-id
            const byId = this.findByResourceId(xmlData, 'com.instagram.android:id/row_feed_button_like');
            if (byId) return { ...byId, alreadyLiked: false };

            // Fallback: content-desc="Like"
            const byDesc = this.findByContentDesc(xmlData, 'Like');
            if (byDesc) return { ...byDesc, alreadyLiked: false };
    ...
    private findCommentButton(xmlData: string): { x: number, y: number } | null {
        const byId = this.findByResourceId(xmlData, 'com.instagram.android:id/row_feed_button_comment');
        if (byId) return byId;
    ...
    private async smartInputText(text: string, inputX: number, inputY: number): Promise<boolean> {
        ...
        const xmlAfterA = await this.device.getUiDumpXml();
        ...
        const xmlAfterB = await this.device.getUiDumpXml();
    ...
    public async likePost(postUrl: string): Promise<boolean> {
        ...
        if (!likeResult) {
            this.ensureLogsDir();
            fs.writeFileSync(`./logs/${this.deviceId}_ig_like_fail.xml`, await this.device.getUiDumpXml());
        ...
        const verifyXml = await this.device.getUiDumpXml();
    ...
    public async commentOnPost(postUrl: string, commentText: string): Promise<boolean> {
        ...
        // Find comment input
        const sheetXml = await this.device.getUiDumpXml();
        ...
        if (!typed) {
            this.ensureLogsDir();
            fs.writeFileSync(`./logs/${this.deviceId}_ig_comment_type_fail.xml`, await this.device.getUiDumpXml());
    ...
    public async engagePost(postUrl: string, commentText: string): Promise<boolean> {
        ...
        if (!typed) {
            this.ensureLogsDir();
            fs.writeFileSync(`./logs/${this.deviceId}_ig_engage_type_fail.xml`, await this.device.getUiDumpXml());
    ...
    private findCommentInput(xmlData: string): { x: number, y: number } | null {
        // Primary: EditText element
        const editTextRegex = /class="android\.widget\.EditText"[^>]*bounds="(\[\d+,\d+\]\[\d+,\d+\])"/;
        ...
        // Fallback A: "Add a comment…" placeholder text
        const placeholderRegex = /text="Add a comment[^"]*"[^>]*bounds="(\[\d+,\d+\]\[\d+,\d+\])"/i;
        ...
        const hintRegex = /(?:hint|text)="(?:Add a comment|Write a comment|Comment)[^"]*"[^>]*bounds="(\[\d+,\d+\]\[\d+,\d+\])"/i;
    ...
    private async submitComment(): Promise<void> {
        const submitXml = await this.device.getUiDumpXml();
