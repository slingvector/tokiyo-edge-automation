...
### 4. Popup Rescue Heuristic (Sub-Agents)
- Updated `PerceptionEngine.ts` to recognize when an OS modal or unexpected popup is blocking the main goal, and instruct it to return a `rescue` action.
- Updated `AutonomousAgent.ts` with an elegant **Recursive Sub-Agent** architecture. When a `rescue` is triggered, the FSM pauses and spins up a brand new, temporary `AutonomousAgent` with the strict goal of *"Dismiss any visible popups, dialogs, alerts, or modals"*. Once dismissed, the primary workflow seamlessly resumes!

## Results
The entire pipeline is complete. The Orchestrator can now dispatch autonomous goal-oriented sessions that self-heal against interruptions. The AI will look at the screen (which is dumped in < 100ms), decide what to tap, and the Edge Agent will tap it natively in < 50ms.
