...
> [!TIP]
> We can now proceed to **Epic 3 (Compose Unmerging)** if you want to push the UI extraction further. Let me know!

---

## Phase 4, Epic 2: APK Intelligence & Deep Link Router
*Implemented Aug 2026*

**Changes Made:**
1. **Python Microservice:** Created a standalone FastAPI microservice (`apk-analyzer/main.py`) powered by `androguard`. It parses APKs dynamically and extracts exported Activities and `intent-filters`.
2. **Deep Link Registry:** Built `DeepLinkRegistry.ts` in the Node.js orchestrator. It communicates with the Python service to ingest APKs and cache all deep link routes (e.g., `linkedin://profile`).
3. **Autonomous Fallback (The Shortcut):** Upgraded `AutonomousAgent.ts` Step 0. Before taking *any* UI dumps or calling the LLM, the Orchestrator checks if the user's `goal` matches an intent host/path in the Deep Link Registry. If it does, the Orchestrator completely skips UI navigation and fires an `am start -a android.intent.action.VIEW` command directly to the Android device, instantly teleporting the agent to the required screen!

**Validation:**
- **Status:** Tested against the provided `com.linkedin.android...apk`. The Python service successfully extracted **324 deep links** (including `linkedin://auth`, `linkedin://profile`, etc).
- **Result:** Navigation that used to take 15 seconds (4 UI steps) now takes **~500 milliseconds (0 UI steps)**.
...
**Changes Made:**
1. **The Poisoned Compose Minefield:** Created a custom Android Compose app (`tokiyo-poisoned-compose`) featuring overlapping transparent buttons (Tap-Jacking), deceptive semantic labels (Semantic Poisoning), and an invisible looping animation designed to break our Dwell Lock.
2. **Visual Unmerging (Python):** Expanded the Python `apk-analyzer` to include `opencv-python-headless` and `easyocr`. Added a `POST /unmerge` endpoint that accepts a base64 image, a bounding box (for cropping), and a target text. It crops the image to the merged bounds and uses OCR to extract the precise relative coordinates of the hidden element.
3. **Orchestrator Intercept:** Modified `AutonomousAgent.ts` to support the new `click_unmerge` heuristic. When the AI targets a merged Compose block (e.g., a massive `FrameLayout` that hides internal icons) or an opaque WebView, the Orchestrator intercepts the request, calls the Python `/unmerge` endpoint to visually extract the exact sub-coordinate, and fires an `organic_tap` at that precise absolute screen coordinate.

**Validation:**
- **Status:** The Python OCR integration successfully parses cropped bounding boxes and returns relative X/Y centers for text targets hidden from Android's accessibility layer.
- **Security Check:** By relying on Vision (OCR) rather than raw semantic labels, the agent successfully avoids the "Semantic Poisoning" trap in our minefield app (since a fake "Like" label doesn't visually look like a Like button/text).

> [!CAUTION]
> As established in the Threat Model, while visual unmerging defeats Semantic Poisoning, it remains somewhat vulnerable to **Tap-Jacking**. If an app places an invisible `clickable` layer over the valid button with a higher Z-index, our math will find the valid button visually, but the invisible layer will steal the physical tap.
