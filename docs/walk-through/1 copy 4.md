...
## Epic 7: Real-Time WebSocket Network Bridge

Replaced the local HTTP server on the Android app with a persistent, low-latency Socket.IO client that communicates directly with the Cloud Orchestrator. 

### What was changed
*   Removed `Ktor Server` dependencies in favor of `io.socket:socket.io-client`.
*   Implemented `AgentBridgeService.kt` which connects to the Orchestrator using Engine.IO/Socket.IO, automatically registers the node, and listens for `dispatch_job` events.
*   Implemented `ShellExecutor.kt` using reflection to invoke Shizuku's hidden API (`newProcess`).
*   Configured the Orchestrator (`Dispatcher.ts` and `Server.ts`) to maintain a real-time WebSocket connection to edge nodes using `socket.io`.
*   Enabled `dataSync` foreground service and cleartext HTTP traffic in the `AndroidManifest.xml`.

### Validation Results
*   **Persistent Connection:** The Android App successfully connects to the Orchestrator, authenticates/registers as `node-1`, and stays connected.
*   **Push Execution:** Pushed a test job from the Orchestrator (`echo "Hello Shizuku!"`) which instantly triggered the `dispatch_job` handler on the device.
*   **Cryptography:** Ed25519 signatures successfully verified the incoming WebSocket payload in real-time.
*   **Telemetry:** The app successfully captured Shizuku execution status and emitted a `telemetry_report` WebSocket event, which the Orchestrator received and logged.
  
*(Note: Shizuku is currently reporting "not bound" because the emulator's Shizuku instance hasn't been re-authorized via Wireless Debugging, but the networking pipeline is fully complete).*
